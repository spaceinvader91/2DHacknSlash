...
public class LockOnFinder : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        bool onScreen =  enemyPosition.x > 0 && enemyPosition.x < 1 && enemyPosition.y > 0 && enemyPosition.y < 1;

        //If The Enemy Is On Screen Add It To The List Of Nearby Enemies Only Once
        if (onScreen && addOnlyOnce)
        {
            addOnlyOnce = false;
            LockOnControls.nearByEnemies.Add(this);
        }
    }
}